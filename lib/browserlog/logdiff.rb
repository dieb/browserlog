  ...
  class Logdiff
    ...
    def initialize(options = {})
      ...
      unless File.exist? @log_filename
        fail "No such file or directory #{options[:file]}"
      end
      fail 'Invalid buffer size (must be > 0)' unless @buffer_size > 0
    ...
    def latest_changes
      ...
      chunks.first.include?(@last_seen) ? unseen_logs(chunks) : []
    end
    ...
    def read_chunk(buffers_from_bottom = 0)
      ...
      remaining = @log_file.size - offset_from_bottom
      return [] if remaining < 0

      amount =  [remaining, @buffer_size].min
      offset = remaining > @buffer_size ? remaining - @buffer_size : 0
